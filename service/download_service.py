...
class DownloadYtDlpService(DownloadAbstractService):
    ...
    async def download_video(self, video_url: str, filter_query: ResolutionFilter, cookie_file: str | None = None):
        proxy = await self.proxy_service.get_all()
        file_path = await asyncio.to_thread(self.download_video_sync, video_url, proxy, filter_query, cookie_file)
    ...
    def download_video_sync(self, video_url: str, proxy_list: List[ProxySchema], filter_query: ResolutionFilter,
        ...
        raise HTTPException(status_code=500, detail="Proxy list dont working")
